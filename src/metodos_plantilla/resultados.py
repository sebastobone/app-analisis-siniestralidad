...
def generar_informe_actuario_responsable(negocio: str, mes_corte: int) -> None:
    ...
    df_atip = (
        ...
        .with_columns([
            ...
            .otherwise(((pl.col("periodo_ocurrencia").cast(pl.Int32)//100)*100)+12)
            ...
            .when(pl.col("periodo_ocurrencia").cast(pl.Int32) + 2 > mes_corte)
            .then(pl.lit("Mensual"))
            .otherwise(pl.lit("Trimestral")).alias("periodicidad_ocurrencia").cast(pl.String)
    ...
    df_bruto = (
        df_unid
        .select(
            ["codigo_ramo_op", "periodicidad_ocurrencia", "periodo_ocurrencia",
             "ibnr_bruto", "ibnr_contable_bruto","plata_ultimate_contable_bruto",
             "plata_ultimate_bruto","pago_bruto_atipico", "aviso_bruto_atipico",
             "pago_bruto", "aviso_bruto"
             ]


        )
        .with_columns(
            pl.lit("bruto").alias("atributo")
        )
    )   
    ...
    df_retenido = (
        ...
        .select(
            ["codigo_ramo_op", "periodicidad_ocurrencia", "periodo_ocurrencia",
             "ibnr_retenido", "ibnr_contable_retenido",
             "plata_ultimate_contable_retenido","plata_ultimate_retenido",
             "pago_retenido_atipico", "aviso_retenido_atipico",
             "pago_retenido", "aviso_retenido"
             ]
        )
        .with_columns(
            pl.lit("retenido").alias("atributo")
        )
    )
    ...
    df_primas = df_primas.lazy()
    df_sinis = df_sinis.lazy()


    df_ar = (
        df_sinis.join(
            df_primas,
            on=["codigo_ramo_op", "periodicidad_ocurrencia",
                 "periodo_ocurrencia","atributo"],
            how="left",
        )
        .fill_null(0)
        .with_columns([
            (pl.col("periodo_ocurrencia").cast(pl.Int32) // 100)
            .cast(pl.Int32).alias("anio_ocurrencia"),


            pl.when((pl.col("periodo_ocurrencia").cast(pl.Int32) % 100) < 7)
            .then(1).otherwise(2)
            .cast(pl.Int32).alias("semestre_ocurrencia"),


            pl.when((pl.col("periodo_ocurrencia").cast(pl.Int32) % 100) < 4).then(1)
            .when((pl.col("periodo_ocurrencia").cast(pl.Int32) % 100) < 7).then(2)
            .when((pl.col("periodo_ocurrencia").cast(pl.Int32) % 100) < 10).then(3)
            .otherwise(4)
            .cast(pl.Int32).alias("trimestre_ocurrencia"),


            pl.when(pl.col("periodicidad_ocurrencia") == "Trimestral").then(-1) \
            .otherwise(pl.col("periodo_ocurrencia").cast(pl.Int32) % 100)
            .cast(pl.Int32).alias("mes_ocurrencia"),
    ...
    .with_columns([
        ...
        for col in [
            "pagos_tipicos", "pagos_atipicos", "aviso_tipico", "aviso_atipico",
            "sue_contable", "sue_actuarial", "ibnr_contable", "ibnr_actuarial",
        ]
    ])
        .select(
            ["codigo_ramo_op","periodo_corte","anio_ocurrencia", "semestre_ocurrencia",
             "trimestre_ocurrencia", "mes_ocurrencia","atributo","pagos_tipicos",
             "pagos_atipicos","aviso_tipico","aviso_atipico","sue_contable",
             "sue_actuarial","ibnr_contable","ibnr_actuarial","prima_devengada"
             ]
        )
        .sort(["codigo_ramo_op", "anio_ocurrencia", "semestre_ocurrencia",
                "trimestre_ocurrencia", "mes_ocurrencia"])
        .collect()
)
