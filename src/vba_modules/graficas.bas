Function grafica_ultimate(ws, num_ocurrencias, nombre_cantidad)
    ...
    Dim grafUlt As Shape

    fila_tabla = fila_tabla_fn(ws)
    
    ws.Range(ws.Cells(fila_tabla + 2, 2), ws.Cells(fila_tabla + num_ocurrencias, 4)).Select
    Set grafUlt = ws.Shapes.AddChart2(227, xlLine)

    grafUlt.Chart.SeriesCollection(1).XValues = ws.Range(ws.Cells(fila_tabla + 2, 1), ws.Cells(fila_tabla + num_ocurrencias, 1))
    grafUlt.Chart.ChartTitle.Text = nombre_cantidad
    
    grafUlt.Left = ws.Cells(fila_tabla - num_ocurrencias, 1).Left
    grafUlt.Top = ws.Cells(fila_tabla - num_ocurrencias, 1).Top
...
Function grafica_analisis_factores()
    ...
    Cells(fila_tabla, 2).Formula2R1C1 = "=+FILTER(FILTER(IF(R2C3 = ""Pago"", R" & fila_ratios + 3 & "C7:R" & fila_ratios + 3 + num_ocurrencias - 1 & "C" & 6 + num_alturas & ", R" & fila_ratios + 3 & "C" & 7 + num_alturas & ":R" & fila_ratios + 3 + num_ocurrencias - 1 & "C" & 6 + num_alturas * 2 & "), IF(R2C3 = ""Pago"", R" & fila_ratios + 2 & "C7:R" & fila_ratios + 2 & "C" & 6 + num_alturas & ", R" & fila_ratios + 2 & "C" & 7 + num_alturas & ":R" & fila_ratios + 2 & "C" & 6 + num_alturas * 2 & ") = R" & fila_exclusiones - 2 & "C2), R" & fila_ratios + 3 & "C5:R" & fila_ratios + 3 + num_ocurrencias - 1 & "C5<=( R" & fila_exclusiones - 3 & "C2 + R" & fila_exclusiones - 2 & "C2 ))"
    Cells(fila_tabla, 3).Formula2R1C1 = "=+FILTER(FILTER(IF(R2C3 = ""Pago"", R" & fila_exclusiones + 3 & "C7:R" & fila_exclusiones + 3 + num_ocurrencias - 1 & "C" & 6 + num_alturas & ", R" & fila_exclusiones + 3 & "C" & 7 + num_alturas & ":R" & fila_exclusiones + 3 + num_ocurrencias - 1 & "C" & 6 + num_alturas * 2 & "), IF(R2C3 = ""Pago"", R" & fila_exclusiones + 2 & "C7:R" & fila_exclusiones + 2 & "C" & 6 + num_alturas & ", R" & fila_exclusiones + 2 & "C" & 7 + num_alturas & ":R" & fila_exclusiones + 2 & "C" & 6 + num_alturas * 2 & ") = R" & fila_exclusiones - 2 & "C2), R" & fila_ratios + 3 & "C5:R" & fila_ratios + 3 + num_ocurrencias - 1 & "C5<=( R" & fila_exclusiones - 3 & "C2 + R" & fila_exclusiones - 2 & "C2 ))"
    ...
    Cells(fila_tabla, 12).Formula2R1C1 = "=+FILTER(IF(R2C3 = ""Pago"", R" & fila_fact_sel & "C7:R" & fila_fact_sel & "C" & 6 + num_alturas & ", R" & fila_fact_sel & "C" & 7 + num_alturas & ":R" & fila_fact_sel & "C" & 6 + num_alturas * 2 & "), IF(R2C3 = ""Pago"", R" & fila_ratios + 2 & "C7:R" & fila_ratios + 2 & "C" & 6 + num_alturas & ", R" & fila_ratios + 2 & "C" & 7 + num_alturas & ":R" & fila_ratios + 2 & "C" & 6 + num_alturas * 2 & ") = R" & fila_exclusiones - 2 & "C2)"
